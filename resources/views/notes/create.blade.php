...
<body class="bg-gray-100 min-h-screen">
    <div class="max-w-2xl mx-auto px-4 py-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-6">Create Note</h1>
        ...
        <form action="{{ route('notes.store') }}" method="POST" class="space-y-6 bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            ...
            <div>
                ...
                <textarea name="content" rows="6" class="mt-2 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('content') }}</textarea>
                @error('content')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
